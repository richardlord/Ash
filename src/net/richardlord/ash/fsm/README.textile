...
bc. var fsm : EntityStateMachine = new EntityStateMachine();
fsm.createState( "guard" )
   .add( Patrol ).withInstance( new Patrol( guardPath ) );
fsm.createState( "investigate" )
   .add( Investigate ).withType( Investigate );
fsm.createState( "defend" )
   .add( Defend ).withType( Defend );
fsm.changeState( "guard" );
entity.add( fsm );

bc. game.addSystem( new EntityStateMachineSystem(), 10 );

h2. The methods

A finite state machine is a component. It is an instance of the EntityStateMachine class and is added as a component 
to the entity whose state it manages. It is configured with states, and the state can be changed by calling its
@changeState()@ method. States are identified by a string, which is assigned when the state is created and used to
identify the state when calling the @changeState()@ method.
...
The state machine relies on the EntityStateMachineSystem to actually change the state of the entity, so this system
must be added to your game. @changeState()@ may be called form any system, but the state won't actually change until
the EntityStateMachineSystem updates the state machine.
...
i.e. if they would provide an equivalent entity.
