...
entity.addStateMachine( "guardStates", fsm );

h2. The methods

A finite state machine is a property of the entity. It is an instance of the EntityStateMachine class and is added
to the entity whose state it manages. It can be recovered from the entity by passing its name to the @getStateMachine()@
method of the Entity class.

A state machine is configured with states, and the state can be changed by calling its @changeState()@ method. States 
are identified by a string, which is assigned when the state is created and used to identify the state when calling 
...
entity.addStateMachine( "lifeCycle", fsm );
